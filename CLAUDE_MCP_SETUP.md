...
1. Go to `https://acko-gen.vercel.app` in a browser.
...
cat > .mcp.json << 'EOF'
{
  "mcpServers": {
    "acko-image-gen": {
      "type": "http",
      "url": "https://acko-gen.vercel.app/mcp",
      "headers": {
...
```bash
claude mcp add acko-image-gen --transport http https://acko-gen.vercel.app/mcp --header "Authorization: Bearer YOUR_TOKEN_HERE" --scope user
```
