...
saved to Supabase Storage by main.py's save_generated_bytes(). This module
indexes that metadata in Postgres (same database user_store.py uses) so it
can be listed and shared across every user. Images themselves live in
Supabase; the `image_url` column holds the permanent public URL directly,
since the bucket is Public — no per-read re-signing needed.
